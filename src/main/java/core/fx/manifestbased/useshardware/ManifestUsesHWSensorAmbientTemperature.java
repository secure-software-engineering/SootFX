...
public class ManifestUsesHWSensorAmbientTemperature implements ManifestFeatureExtractor<Boolean> {

    @Override
    public Feature<Boolean> extract(ProcessManifest target) {
        List<String> uses = FxUtil.getManifestUsesFeature(target);
        return new Feature<>(getName(), uses.contains("android.hardware.sensor.ambient_temperature"));
    }
